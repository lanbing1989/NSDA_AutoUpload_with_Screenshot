...
Global $SCREENSHOT_DIR     = IniRead($ConfigFile, "coords", "SCREENSHOT_DIR", @ScriptDir & "\StepScreenshots")
...
Global $inpScreenshotDir, $btnBrowseScreenshot

Local $y1 = 170, $y2 = 210, $y3 = 250, $y4 = 290
...
; 新增第四行：截图保存路径
GUICtrlCreateLabel("截图目录", $x, $y4, $w_label, 18)
$inpScreenshotDir = GUICtrlCreateInput($SCREENSHOT_DIR, $x+$w_label, $y4, 280, 18)
$btnBrowseScreenshot = GUICtrlCreateButton("浏览", $x+$w_label+280+10, $y4, $w_btn, 18)

$lblPick = GUICtrlCreateLabel("点击对应“拾取”按钮，然后在目标位置按F9，将当前位置坐标自动填入该项。", 60, 340, 800, 20)
...
While 1
    ...
    Switch $msg
        ...
        Case $btnBrowseScreenshot
            Local $chosen = FileSelectFolder("选择截图保存目录", "", 1, GUICtrlRead($inpScreenshotDir))
            If $chosen <> "" Then
                GUICtrlSetData($inpScreenshotDir, $chosen)
            EndIf
    EndSwitch
    ...
    If $iPicking > 0 Then
        ...
        If $pos[0] <> $iPickLastX Or $pos[1] <> $iPickLastY Then
            Switch $iPicking
                Case 1: GUICtrlSetData($inpRefresh, $pos[0]&","&$pos[1])
                Case 2: GUICtrlSetData($inpSelectAll, $pos[0]&","&$pos[1])
                Case 3: GUICtrlSetData($inpC5Game, $pos[0]&","&$pos[1])
                Case 4: GUICtrlSetData($inpPricing, $pos[0]&","&$pos[1])
                Case 5: GUICtrlSetData($inpConfirm, $pos[0]&","&$pos[1])
                Case 6: GUICtrlSetData($inpFailConfirm, $pos[0]&","&$pos[1])
            EndSwitch
            $iPickLastX = $pos[0]
            $iPickLastY = $pos[1]
        EndIf
...
Func _LoadCoords()
    ...
    $SCREENSHOT_DIR     = GUICtrlRead($inpScreenshotDir)
EndFunc
...
Func _SaveCoords()
    ...
    IniWrite($ConfigFile, "coords", "SCREENSHOT_DIR", GUICtrlRead($inpScreenshotDir))
EndFunc
...
; ===================== 截图函数 =====================
Func SaveStepScreenshot($stepName)
    Local $now = @YEAR & @MON & @MDAY & "_" & @HOUR & @MIN & @SEC
    Local $saveDir = $SCREENSHOT_DIR
    If Not FileExists($saveDir) Then DirCreate($saveDir)
    Local $filename = $saveDir & "\" & $stepName & "_" & $now & ".jpg"
